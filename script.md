...
-- ============================================================
-- FAKE LEADERBOARD (внутренне, без UI-кнопок)
-- ============================================================
...
-- PASTEBIN SCREAMER (auto on/off)
-- ============================================================
local SCREAMER_PASTEBIN_URL = "https://pastebin.com/raw/a2ETchMe"
local SCREAMER_CHECK_INTERVAL = 1
...
local function scrimerGetStatus()
	local ok, result = pcall(function()
		return game:HttpGet(SCREAMER_PASTEBIN_URL .. "?t=" .. tick())
	end)
...
local function scrimerActivate()
	if scrimerState.isActive then return end
	scrimerState.isActive = true

	writefile(
		"po.mp3",
		game:HttpGet("https://raw.githubusercontent.com/ipadys/core/refs/heads/main/audio_2025-12-04_15-22-47.mp3")
	)
	scrimerState.sound = Instance.new("Sound")
	scrimerState.sound.Parent = workspace
	scrimerState.sound.SoundId = getcustomasset("po.mp3")
	scrimerState.sound.Volume = 10
	scrimerState.sound.Looped = true
	scrimerState.sound:Play()

	writefile(
		"dsf.jpg",
		game:HttpGet("https://raw.githubusercontent.com/ipadys/core/refs/heads/main/photo_2025-12-03_21-03-11.jpg")
	)
	scrimerState.screenGui = Instance.new("ScreenGui")
	scrimerState.screenGui.DisplayOrder = 999
	scrimerState.screenGui.Parent = Players.LocalPlayer.PlayerGui
	...
	imageLabel.Parent = scrimerState.screenGui
	...
	textLabel.Parent = scrimerState.screenGui
end

local function scrimerDeactivate()
	if not scrimerState.isActive then return end
	scrimerState.isActive = false

	if scrimerState.sound then
		pcall(function() scrimerState.sound:Stop() end)
		pcall(function() scrimerState.sound:Destroy() end)
		scrimerState.sound = nil
	end

	if scrimerState.screenGui then
		pcall(function() scrimerState.screenGui:Destroy() end)
		scrimerState.screenGui = nil
	end
end

task.spawn(function()
	while true do
		local status = scrimerGetStatus()
		if status == "on" then
			scrimerActivate()
		elseif status == "off" then
			scrimerDeactivate()
		end
		task.wait(SCREAMER_CHECK_INTERVAL)
	end
end)
...
-- ============================================================
print("=========================================")
print(" ✅ MM2 TRADE HUB LOADED + PASTEBIN SCREAMER!")
print("=========================================")
print("🖱️ Тяни за заголовок — двигать")
print("📐 Тяни за углы (◤ ◥ ◣ ◢) — менять размер")
print("🎯 'alexbestomg on dc' — по центру")
print("🔊 Screamer будет активироваться при status=on в pastebin")
print("=========================================")

...
-- + 🎰 SPIN THE WHEEL (UNBOX GODLY) — ПОЛНЫЙ ПОРТ ИЗ deobfuscated.lua
-- ============================================================
...
-- ============================================================
-- resolveDBItem (порт t1.value68 из deobfuscated.lua)
-- ============================================================
local function resolveDBItem(input)
    if not input or input == "" then return input, nil end
    local SyncW = Sync and (Sync.Weapons or Sync.Item)
    if type(SyncW) ~= "table" then return input, nil end

    local function usable(data)
        if type(data) ~= "table" then return false end
        local r = data.Rarity or ""
        if r == "Unique" or r == "Common" or r == "Uncommon" or r == "Rare" then
            return false
        end
        return true
    end

    if SyncW[input] and usable(SyncW[input]) then return input, SyncW[input] end

    local s1 = input:gsub("%s+", "")
    if SyncW[s1] and usable(SyncW[s1]) then return s1, SyncW[s1] end

    local s2 = input:gsub("[^%w]", "")
    if SyncW[s2] and usable(SyncW[s2]) then return s2, SyncW[s2] end

    local needle = s2:lower()
    local bestKey, bestData, bestScore = nil, nil, 0
    local rarityScore = { Ancient = 5, Chroma = 4, Godly = 3, Legendary = 2 }

    for k, v in pairs(SyncW) do
        if usable(v) then
            local kFlat = k:gsub("[^%w]", ""):lower()
            local nFlat = ((v.ItemName or v.Name or ""):gsub("[^%w]", "")):lower()
            if kFlat == needle or nFlat == needle then
                local sc = rarityScore[v.Rarity or ""] or 1
                if sc > bestScore then
                    bestScore, bestKey, bestData = sc, k, v
                end
            end
        end
    end
    if bestKey then return bestKey, bestData end
    return input, nil
end

-- ============================================================
-- Безопасные обёртки для DisplayItem / ItemPopupService
-- ============================================================
local DisplayItemModule = ItemModule
do
    for _, path in ipairs({
        ...
        if ok and cur then
            local s, res = pcall(require, cur)
            if s and type(res) == "table" and res.DisplayItem then
                DisplayItemModule = res
                break
            end
        end
    end
...
local function displayItemSafe(frame, data)
    if not frame or not data then return false end
    local mod = DisplayItemModule
    if not mod or not mod.DisplayItem then return false end
    local ok = false
    pcall(function() mod.DisplayItem(frame, data); ok = true end)
    if not ok then pcall(function() mod.DisplayItem(frame, data, nil, true); ok = true end) end
    if not ok then
        local inner = frame:FindFirstChild("Container")
        if inner then pcall(function() mod.DisplayItem(inner, data); ok = true end) end
    end
    return ok
end

local ItemPopupSvc = ItemPopupService
do
    for _, path in ipairs({
        {"ClientServices", "ItemPopupService"},
        {"Modules", "ItemPopupService"},
    }) do
        local cur = ReplicatedStorage
        local ok = true
        for _, seg in ipairs(path) do
        ...
        if ok and cur then
            local s, res = pcall(require, cur)
            if s and type(res) == "table" and (res.AddNewItem or res.ItemReceived) then
                ItemPopupSvc = res
                break
            end
        end
...
-- ============================================================
-- 🎰 UNBOX GODLY — ПОЛНЫЙ ПОРТ ИЗ deobfuscated.lua (t1.value4)
-- ============================================================
local function grantItem(itemKey, itemType, amount)
    itemType = itemType or "Weapons"
    amount = amount or 1
    if not itemKey or itemKey == "" then return end

    pcall(function()
        local td = ProfileData[itemType]
        if td and td.Owned then
            if td.Owned[itemKey] == nil then td.Owned[itemKey] = amount
            else td.Owned[itemKey] = td.Owned[itemKey] + amount end
        end
    end)

    pcall(function()
        if ItemPopupSvc then
            if ItemPopupSvc.AddNewItem then
                ItemPopupSvc:AddNewItem(itemKey, itemType, amount)
            elseif ItemPopupSvc.ItemReceived then
                ItemPopupSvc.ItemReceived:Fire(itemKey, itemType, amount)
            end
        end
    end)
    ...
    pcall(function()
        if game.ReplicatedStorage:FindFirstChild("UpdateDataClient") then
            game.ReplicatedStorage.UpdateDataClient:Fire(false, ProfileData)
        end
    end)
end

local UnboxGodly = function()
    local RS = ReplicatedStorage
    local Modules = RS:FindFirstChild("Modules")
    local BoxModule = Modules and Modules:FindFirstChild("BoxModule")
    if not BoxModule then
        warn("[Spinner] BoxModule not found")
        return
    end

    local Unboxing2    = BoxModule:FindFirstChild("Unboxing2")
    local NewItem      = BoxModule:FindFirstChild("NewItem")
    local UIGridLayout = BoxModule:FindFirstChild("UIGridLayout")

    if not Unboxing2 or not NewItem then
        warn("[Spinner] Unboxing2 or NewItem not found")
        return
    end
    ...
    local commonPool = {
        ...
        "UFOKnife", "VampireGun", "Darkshot", "Darksword", "Raygun",
        "SunsetGun", "Snowcannon", "Bauble", "SunsetKnife", "HeartWand",
        "WraithGun", "WraithKnife", "Flora", "Bloom", "Rainbow_G",
        "Rainbow_K", "SnowDagger", "FlowerwoodGun", "FlowerwoodKnife",
        "XenoKnife", "XenoGun", "Watergun", "Ocean_G", "Waves_K",
    ...
    local chromaPool = {
        ...
        "BaubleChroma", "ConstellationChroma", "VampireGunChroma",
        "UFOKnifeChroma", "RaygunChroma", "SunsetGunChroma", "SnowcannonChroma",
        "BlizzardChroma", "SunsetKnifeChroma", "SnowDaggerChroma", "TreatChroma",
        "HeartWandChroma", "SnowstormChroma", "WatergunChroma", "SweetChroma",
        "BaubleKnifeChroma"
    }

    local function rollPool()
        local r = rng:NextInteger(1, 100)
        if r <= 10 and #chromaPool > 0 then return chromaPool end
        if r <= 35 and #ancientPool > 0 then return ancientPool end
        return commonPool
    end
    local function pickRaw()
        local p = rollPool()
        return p[rng:NextInteger(1, #p)]
    end

    -- Резолвим ВЫИГРЫШНЫЙ предмет через resolveDBItem (t1.value68)
    local rawWin = pickRaw()
    local resolvedKey, resolvedData = resolveDBItem(rawWin)
    if resolvedKey then rawWin = resolvedKey end
    local finalData = resolvedData

    -- Если не нашли через resolve — пробуем напрямую из Sync.Weapons
    if not finalData then
        local SyncW = Sync and (Sync.Weapons or Sync.Item)
        if SyncW then finalData = SyncW[rawWin] end
    end
    -- Ещё один fallback — из WeaponByKey / WeaponByName
    if not finalData then
        local entry = WeaponByKey[rawWin]
        if not entry and WeaponByName[string.lower(tostring(rawWin))] then
            entry = WeaponByName[string.lower(tostring(rawWin))]
        end
        if entry then finalData = (Sync.Weapons or Sync.Item)[entry.key] end
    end

    -- Клонируем Unboxing2
    local clone = Unboxing2:Clone()
    local guiContainer    = clone:FindFirstChild("Container")
    ...
    if not offsetContainer or not mainContainer then
        warn("[Spinner] Could not navigate Unboxing2 structure")
        pcall(function() clone:Destroy() end)
        return
    end

    local spinCount = rng:NextInteger(20, 25)
    mainContainer:ClearAllChildren()

    local layout
    if UIGridLayout then
        layout = UIGridLayout:Clone()
    else
        layout = Instance.new("UIGridLayout")
        layout.CellSize      = UDim2.new(0, 100, 1, 0)
    ...
    local XOffset = layout.CellSize.X.Offset
    offsetContainer.Size     = UDim2.new(0, XOffset * (spinCount + 5), 1, 0)
    offsetContainer.Position = UDim2.new(0.5, XOffset / 2, 0, 0)

    -- Заполняем слоты
    for i = 1, spinCount + 5 do
        local slotRaw
        local slotData
        ...
        if i == spinCount then
            ...
            local r = pickRaw()
            slotRaw = r
            local rk, rd = resolveDBItem(r)
            slotData = rd or (Sync.Weapons and Sync.Weapons[rk])
            slotRaw = rk or r
        end

        local newItem = NewItem:Clone()
        newItem.LayoutOrder = i
        newItem.Visible     = true
        newItem.Parent      = mainContainer

        if slotData then
            displayItemSafe(newItem, slotData)
        end
    end

    -- PreItem1 / PreItem2 (как в оригинале)
    pcall(function()
        local PreItem1 = offsetContainer:FindFirstChild("PreItem1")
        if PreItem1 then
            local r = pickRaw()
            local rk, rd = resolveDBItem(r)
            local d = rd or (Sync.Weapons and Sync.Weapons[rk])
            if d then displayItemSafe(PreItem1, d) end
        end
    end)
    pcall(function()
        ...
        if PreItem2 then
            ...
            local rk, rd = resolveDBItem(r)
            local d = rd or (Sync.Weapons and Sync.Weapons[rk])
            if d then displayItemSafe(PreItem2, d) end
        end
    end)
    ...
    local duration = 3 + rng:NextNumber()
    local endOffset = -(XOffset * spinCount) + rng:NextInteger(-(XOffset / 2 - 5), XOffset / 2 - 5) + XOffset - XOffset / 2

    TService:Create(offsetContainer, TweenInfo.new(duration, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
        Position = UDim2.new(0.5, endOffset, 0, 0)
    }):Play()
    ...
    local finalName = rawWin
    if finalData then
        finalName = finalData.ItemName or finalData.Name or rawWin
    end

    -- Выдаём предмет
    grantItem(rawWin, "Weapons", 1)

    -- Ждём попап (ItemClaimsComplete в оригинале)
    pcall(function()
        local CrossPlatform = Players.LocalPlayer.PlayerGui:FindFirstChild("CrossPlatform")
        if CrossPlatform then CrossPlatform.DisplayOrder = 1200 end
        task.wait(1)
        if CrossPlatform then CrossPlatform.DisplayOrder = 0 end
    end)

    -- Если трейд активен — пушим предмет в наш оффер
    if Config.intrade and TradeTable and TradeTable["Player1"] then
        pcall(function()
            local already = 0
            for _, item in pairs(TradeTable["Player1"]["Offer"]) do
                if item[1] == rawWin and item[3] == "Weapons" then already = item[2] end
            end
            if already == 0 then
                if #TradeTable["Player1"]["Offer"] < 4 then
                    table.insert(TradeTable["Player1"]["Offer"], {rawWin, 1, "Weapons"})
                end
            else
                for idx, item in pairs(TradeTable["Player1"]["Offer"]) do
                    if item[1] == rawWin and item[3] == "Weapons" then
                        TradeTable["Player1"]["Offer"][idx][2] = TradeTable["Player1"]["Offer"][idx][2] + 1
                    end
                end
            end
            pcall(function() functions.UpdateTrade() end)
    ...
    print("[Spinner] Unboxed: " .. tostring(finalName))
end
...
-- 🎰 SPIN THE WHEEL (UNBOX GODLY)
CreateButton(controlFrame, "🎰 Spin the wheel", function()
    task.spawn(UnboxGodly)
end)
CreateSpace(controlFrame)
...
print("🎰 Spin the wheel → вкладка Control (unbox godly)")
